...
Figure 6 shows the Boosted Tree's performance for the categories: Mandela, Mbeki, Ramaphosa, and Zuma. The diagonal entries represent correct classifications: 93 for Mandela, 278 for Mbeki, 253 for Ramaphosa, and 385 for Zuma, indicating varying degrees of accuracy across classes. Notably, the Boosted Tree's is most accurate when predicting Zuma and least accurate for Mandela. The off-diagonal elements reveal misclassifications. For instance, Mandela is often mistaken as Mbeki (98 times) and Zuma (100 times). Similarly, Zuma is misclassified as Ramaphosa 67 times. 
...
# Obtain model's predictions on the test set
y_pred = loaded_model.predict(X_test)
...
The confusion matrix of the SVM classifier indicates varying classification accuracy for the different classes. For the Mandela class, there were 139 correct predictions out of 334 instances, resulting in a classification accuracy of 41.6%. The Mbeki class had an accuracy of 54.8%, with 263 correct predictions out of 480 instances. The classifier was more accurate in identifying Ramaphosa, correctly classifying 271 out of 456 instances, yielding an accuracy of 59.4%. The highest classification accuracy was observed for the Zuma class, with 393 correct predictions out of 526 instances, achieving a 74.7% accuracy rate.

Patterns of misclassification were also observed across the classes. For instance, Mandela was often misclassified as Mbeki (80 instances) and Zuma (77 instances). Instances of Mbeki were misclassified as Mandela (71 instances) and Zuma (99 instances). Ramaphosa was misclassified as Zuma in 126 instances, with smaller numbers for Mandela and Mbeki. The Zuma class had instances misclassified as Ramaphosa (64), Mandela (25), and Mbeki (44). 
```{python}
#| echo: false
# Compute the confusion matrix
...
The results of the training over epochs for the GBT model using TF-IDF can be seen in the appendix(Figure XXX), though is largely the same in behaviour as the GBT model trainined usinig the BoW feature extraction. Hence the final model chosen has the same hyperpararmeters. 
...
The confusion matrix for the Gradient Boosting Trees (GBT) classifier reveals a classification accuracy of 28.7% for Mandela, with 96 correct predictions out of 334 instances. The Mbeki class demonstrated a higher accuracy of 59.2% with 284 correct predictions from 480 instances. The Ramaphosa class was accurately identified in 261 out of 456 instances, yielding an accuracy of 57.2%. The Zuma class exhibited the highest classification accuracy at 68.3%, with 359 correct predictions from 526 instances.

In terms of misclassifications, Mandela was often confused with Mbeki (113 instances) and Zuma (82 instances). Instances labeled as Mbeki were misclassified as Mandela (44 instances), Ramaphosa (60 instances), and Zuma (92 instances). There were 125 instances of Ramaphosa misclassified as Zuma, with additional misclassifications into Mandela (18 instances) and Mbeki (52 instances). The Zuma class saw misclassifications into Mandela (18 instances), Mbeki (65 instances), and Ramaphosa (84 instances). These misclassification patterns provide important insights for future model refinement and improvement.
...
In Figure XXX for the SVM classifier using TF- IDF feature extraction the confusion matrix denotes: Mandela at 42.8%, with 143 correct predictions out of 334; Mbeki at 61.9%, with 297 correct predictions out of 480; Ramaphosa at 64.3%, with 293 correct predictions out of 456; and Zuma at 70.5%, with 371 correct predictions out of 526.

In terms of misclassifications, 94 Mandela instances were classified as Mbeki, 41 as Ramaphosa, and 56 as Zuma. For the Mbeki class, 59 instances were misclassified as Mandela, 44 as Ramaphosa, and 80 as Zuma. The classifier misidentified 23 Ramaphosa instances as Mandela, 31 as Mbeki, and 109 as Zuma. Finally, in the Zuma category, there were 25 instances misclassified as Mandela, 55 as Mbeki, and 75 as Ramaphosa.
...
In Figure XXX we look to the confusion matrix of the GBT using word embeddings. For Mandela, there were 38 correct predictions from 334 instances, translating to an accuracy of 11.4%. The Mbeki class had 10 correct predictions from 480 instances, resulting in an accuracy of just 2.1%. The classifier showed a stronger performance for the Ramaphosa class, correctly identifying 342 out of 526 instances, yielding an accuracy of 65.0%. The Zuma class had 163 correct predictions from 456 instances, resulting in an accuracy of 35.7%.

Misclassifications were also evident. Notably, 183 Mandela instances were misclassified as Ramaphosa and 111 as Zuma. For Mbeki, the classifier misidentified 244 instances as Ramaphosa and 150 as Zuma. The Ramaphosa class had 141 instances misclassified as Zuma, with smaller numbers of misclassifications into Mandela (39) and Mbeki (4). For Zuma, there were 254 instances misclassified as Ramaphosa, 36 as Mandela, and 3 as Mbeki.
...
In Figure XXX we see the word embeddnigs SVM confusion matrix: For the Mandela class, 13 correct predictions were made out of 334 instances, yielding a classification accuracy of 3.9%. The Mbeki class observed a more substantial accuracy of 61.5, with 295 correct predictions from 480 instances. The classifier identified the Ramaphosa class accurately in 269 out of 526 instances, resulting in a 51.1% accuracy rate. The Zuma class had 142 correct predictions out of 456 instances, translating to an accuracy of 31.1%.

Regarding misclassifications, a significant number of Mandela instances, 178, were misclassified as Mbeki, followed by 79 as Ramaphosa and 64 as Zuma. In the Mbeki class, 117 instances were identified as Ramaphosa and 55 as Zuma, with a smaller misclassification into Mandela (13 instances). For the Ramaphosa class, 149 instances were misclassified as Mbeki, 94 as Zuma, and a minor count into Mandela (14 instances). For the Zuma class, there were notable misclassifications into Mbeki (141 instances) and Ramaphosa (151 instances), with a smaller number into Mandela (22 instances). 
...
sns.set(font_scale=1.2)  # for label size
sns.heatmap(cm_specific_svm, annot=True, fmt='g', cmap='Blues', annot_kws={"size": 12}, ax=ax_specific_svm,xticklabels=classes, yticklabels=classes)  # font size
...
We finally examine the confusion matrix for the NN using word embeddings in figure XXX. For the Mandela class, there were no correct predictions from the 334 instances, resulting in a classification accuracy of 0%, notably the model did not predict a single sentence as belonging to Madela. The Mbeki class secured a higher accuracy of 68.5%, with 329 correct predictions out of 480 instances. The Ramaphosa class was correctly identified in 283 out of 526 instances, translating to an accuracy of 53.8%. The Zuma class saw 47 correct predictions from 456 instances, yielding an accuracy of 10.3%. In a similar vein as the SVM, we find that the model tends to

In terms of misclassifications, the Mandela class was predominantly misclassified as Mbeki (196 instances), followed by Ramaphosa (111 instances) and Zuma (27 instances). For the Mbeki class, there were 131 instances misclassified as Ramaphosa and 20 as Zuma. In the Ramaphosa class, a significant 214 instances were misclassified as Mbeki, with 29 as Zuma. Lastly, the Zuma class had substantial misclassifications into Mbeki (205 instances) and Ramaphosa (204 instances). These patterns highlight areas of misclassification and suggest potential avenues for improving the SVM classifier's performance. Overall this model seemed to overpredict sentences as belonging to either Mbeki or Ramaphosa.

```{python}
#| echo: false
y_pred_nn3 = loaded_model.predict(X_test)
y_pred_nn3_classes = np.argmax(y_pred_nn3, axis=1)  # Convert probabilities to class labels
